...
try {
  ...
  const experience = await readFile(join(root, "work-and-experience.html"), "utf8");
  const customers = await readFile(join(root, "who-we-work-with.html"), "utf8");
  const shop = await readFile(join(root, "shop.html"), "utf8");
  const allTerrain = await readFile(join(root, "case-studies/all-terrain-projects.html"), "utf8");
  const responsibleAi = await readFile(join(root, "insights/responsible-ai-assurance.html"), "utf8");
  ...
  requireText(foundations, ["AI Foundations", "Shared business context", "human approval rules", "That belongs in AI Workflow", "Relevant experience"], "services/ai-embedment.html");
  requireText(workflow, ["AI Workflow in practice", "The trigger", "human approvals", "exception paths", "version control", "Relevant experience"], "services/agent-workflow.html");
  requireText(contact, ["value=\"ai-embedment\">AI Foundations", "value=\"agent-workflow\">AI Workflow"], "contact.html");
  requireText(experience, ["Geordie Lindsay-Russell", "Training and facilitation", "Qualifications and professional development", "OpenAI Academy Courses"], "work-and-experience.html");
  requireText(customers, ["The Physio Bunker", "HoverWorks", "Environment Control Products", "All Terrain Projects", "BADinz", "Core platform experience", "Broader connector ecosystem"], "who-we-work-with.html");
  requireText(shop, ["AI Advisory", "Coming soon", "Responsible AI Assurance", "Creating Reusable Skills", "Cloud, Local and App AI"], "shop.html");
  requireText(allTerrain, ["All Terrain Projects", "The business problem", "AI supported operating system", "Human controls", "Measures to validate"], "case-studies/all-terrain-projects.html");
  requireText(responsibleAi, ["Responsible AI assurance", "12 August 2026", "Start with the work", "Keep people accountable", "Business.govt.nz"], "insights/responsible-ai-assurance.html");
  ...
  requireText(clientScript, ["cta_select", "faq_open", "contact_start", "contact_submit", "enquiry_complete", "generate_lead", "data-testimonial-slider"], "script.js");
